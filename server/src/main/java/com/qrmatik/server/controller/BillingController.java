...
public class BillingController {
  ...
  public ResponseEntity<?> initializeCheckout(
      @Valid @RequestBody CheckoutInitRequest req, HttpServletRequest http) {
    ...
    try {
      var init =
          billingService.initializeHostedCheckout(
              tOpt.get(), req.getPlan(), req.getBillingPeriod(), baseUrl);
  ...
  public ResponseEntity<?> scheduleDowngrade(@Valid @RequestBody SchedulePlanChangeRequest req) {
    ...
    String targetPeriod =
        req.getBillingPeriod() == null ? null : req.getBillingPeriod().trim().toUpperCase();
    if (targetPeriod != null && !"YEARLY".equalsIgnoreCase(targetPeriod)) {
      return ResponseEntity.badRequest()
          .body(Map.of("message", "Sadece yıllık faturalama desteklenmektedir"));
    }
    if (targetPlan == null)
    ...
    if (t.getPlanPaidUntil() == null) {
      ...
      return ResponseEntity.ok(
          Map.of("applied", true, "plan", targetPlan.name(), "billingPeriod", targetPeriod));
    }
