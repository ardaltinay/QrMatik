...
public class PublicSignupController {
  ...
  public ResponseEntity<?> signup(@RequestBody PublicTenantSignupRequest req) {
    ...
    if (req.getCode() == null
        || req.getCode().isBlank()
        || req.getOwnerName() == null
        || req.getOwnerName().isBlank()
        || req.getOwnerEmail() == null
        || req.getOwnerEmail().isBlank()
        || req.getPrimaryColor() == null
        || req.getPrimaryColor().isBlank()
        || req.getAccentColor() == null
    ...
    if ((req.getAdminUsername() != null && !req.getAdminUsername().isBlank())
      ...
      if (req.getAdminUsername() != null
          && !req.getAdminUsername().isBlank()
      ...
      if (req.getKitchenUsername() != null
          && !req.getKitchenUsername().isBlank()
          && req.getKitchenPassword() != null
          && !req.getKitchenPassword().isBlank()) {
        boot.setKitchenUsername(req.getKitchenUsername());
        boot.setKitchenPassword(req.getKitchenPassword());
      }
      if (req.getBarUsername() != null
          && !req.getBarUsername().isBlank()
          && req.getBarPassword() != null
