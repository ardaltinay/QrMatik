...
public class OrderController {
  ...
  public ResponseEntity<OrderDto> get(
      ...
      @org.springframework.web.bind.annotation.RequestParam(name = "sid", required = false)
          String sid) {
  ...
  public ResponseEntity<OrderDto> updateStatus(
      @PathVariable String id, @RequestBody StatusUpdate payload) {
    String tenant = TenantContext.getTenant();
    Optional<OrderEntity> updated = orderService.updateStatus(id, tenant, payload.getStatus());
    return updated
        .map(orderEntity -> ResponseEntity.ok(converter.toDto(orderEntity)))
  ...
  public ResponseEntity<?> bySession(@PathVariable String sessionId) {
    ...
    for (OrderEntity e : entities) {
      boolean isExpired =
          (e.getSessionExpiresAt() != null && e.getSessionExpiresAt().isBefore(now));
      if (!isExpired) {
        anyNonExpired = true;
        break;
      }
  ...
  public ResponseEntity<?> requestBill(
      @PathVariable String id, @RequestBody RequestBillRequest req) {
    String tenant = TenantContext.getTenant();
    String sessionId = (req != null ? req.getSessionId() : null);
