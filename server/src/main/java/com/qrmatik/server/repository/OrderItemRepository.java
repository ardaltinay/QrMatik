...
public interface OrderItemRepository extends JpaRepository<OrderItemEntity, UUID> {
    ...
    @Query("select li.menuItem.id as id, sum(coalesce(li.quantity,0)) as qty " +
        "from OrderItemEntity li " +
        "where (:tenant is null or (li.order.tenant is not null and li.order.tenant.code = :tenant)) " +
        "and (li.order.status <> :canceled) " +
        "group by li.menuItem.id order by qty desc")
    List<Object[]> topMenuItemCounts(@Param("tenant") String tenant, @Param("canceled") OrderStatus canceled, Pageable pageable);
}
