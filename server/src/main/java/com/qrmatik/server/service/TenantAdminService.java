...
public class TenantAdminService {
  private static final Pattern CODE_RE =
      Pattern.compile("^(?=.{1,63}$)[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$");
  ...
  public Optional<TenantEntity> create(TenantInsertRequest req) {
    if (req.getCode() == null || req.getCode().isBlank()) return Optional.empty();
    String code = req.getCode().trim();
    if (!CODE_RE.matcher(code).matches()) return Optional.empty();
    if (tenantRepository.findByCode(code).isPresent()) return Optional.empty();
    ...
    TenantEntity e =
        TenantEntity.builder()
            ...
            .plan(plan != null ? plan : PlanType.FREE)
            .build();
  ...
  public Optional<TenantEntity> update(String id, TenantInsertRequest req) {
    ...
    if (req.getOwnerEmail() != null) e.setOwnerEmail(req.getOwnerEmail());
    if (req.getLogoUrl() != null) e.setLogoUrl(req.getLogoUrl());
    if (req.getPrimaryColor() != null) e.setPrimaryColor(req.getPrimaryColor());
    if (req.getAccentColor() != null) e.setAccentColor(req.getAccentColor());
    if (req.getConfig() != null) e.setConfigJson(req.getConfig());
    if (req.getPlan() != null) {
      PlanType plan = PlanType.fromString(req.getPlan());
      if (plan != null) e.setPlan(plan);
  ...
  private void insertUser(TenantEntity tenant, String username, String role, String rawPassword) {
    var existing =
        userRepository.findTopByUsernameAndTenant_CodeOrderByCreatedTimeDesc(
            username, tenant.getCode());
    UserEntity u =
        existing.orElseGet(() -> UserEntity.builder().username(username).tenant(tenant).build());
