...
public class BillingService {
  ...
  public BillingService(
      Options options,
      IyzicoProperties props,
      PricingService pricingService,
      TenantRepository tenantRepository,
      java.time.ZoneId appZoneId) {
  ...
  public InitResponse initializeHostedCheckout(
      TenantEntity tenant, String plan, String period, String baseUrl) {
    if (!props.isEnabled()) {
    ...
    String callbackBase =
        StringUtils.hasText(props.getCallbackBaseUrl()) ? props.getCallbackBaseUrl() : baseUrl;
    ...
    buyer.setEmail(
        tenant.getOwnerEmail() == null || tenant.getOwnerEmail().isBlank()
            ? "default@example.com"
  ...
  public String handleCallbackAndUpgrade(String token) {
    ...
    java.time.LocalDate until =
        "YEARLY".equalsIgnoreCase(period) ? now.plusYears(1) : now.plusMonths(1);
    ...
    t.setLastPaymentAt(java.time.Instant.now());
