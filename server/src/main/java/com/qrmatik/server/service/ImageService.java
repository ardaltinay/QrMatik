...
public class ImageService {
  ...
  public SavedImages saveMenuItemImage(String tenantCode, UUID menuItemId, MultipartFile file)
    ...
    if (ext == null) {
      throw new IOException(
          "Unsupported image type. Please upload JPG/PNG (WEBP supported if enabled).");
    }
    Path base = resolveUploadBase();
    Path dir =
        base.resolve("tenants")
            .resolve(safe(tenantCode))
            .resolve("menu")
            .resolve(String.valueOf(menuItemId));
    log.info(
        "Saving menu image. uploadDir={}, resolvedBase={}, resolvedDir={}",
        uploadDir,
        base.toAbsolutePath(),
        dir.toAbsolutePath());
    ...
    if (!Files.exists(original) || !Files.exists(medium) || !Files.exists(thumb)) {
      log.error(
          "Image generation incomplete. Exists? original={}, medium={}, thumb={}",
    ...
    String baseUrl =
        "/files/tenants/" + safe(tenantCode) + "/menu/" + String.valueOf(menuItemId) + "/";
    return new SavedImages(
        baseUrl + "original." + ext, baseUrl + "medium.jpg", baseUrl + "thumb.jpg");
  }
  ...
  private String detectExt(String originalName, String contentType) {
    ...
    if (originalName.contains(".")) {
      ...
      if (ext.equals("jpg") || ext.equals("png") || ext.equals("webp")) return ext;
    }
