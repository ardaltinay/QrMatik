...
export async function fetchJson(path, opts = {}) {
  const res = await apiFetch(path, Object.assign({}, opts, { headers: Object.assign({'Content-Type':'application/json'}, opts.headers || {}) }))
  if (!res.ok) {
    let msg = 'İstek başarısız: ' + res.status
    try {
      const text = await res.text()
      if (text) {
        try { const j = JSON.parse(text); msg = j.error || j.message || text }
        catch { msg = text }
      }
    } catch { /* ignore */ }
    try {
