...
export const useUserStore = defineStore('user', () => {
  ...
  async function loadUsers() {
    try {
      ...
      users.value = users.value.map((u, idx) => ({ id: u.id || idx + 1, username: u.username, role: u.role }))
    } catch (err) {
  ...
  async function createUser(username, role = 'staff', password) {
    ...
    const created = await fetchJson('/api/users', { method: 'POST', body: JSON.stringify(payload), headers: { 'Content-Type': 'application/json' } })
  ...
  async function updateUser(id, data) {
    ...
    const updated = await fetchJson(`/api/users/${encodeURIComponent(id)}`, { method: 'PUT', body: JSON.stringify(payload), headers: { 'Content-Type': 'application/json' } })
    const u = users.value.find(x => String(x.id) === String(id))
    if (u) { u.username = updated.username; u.role = updated.role }
