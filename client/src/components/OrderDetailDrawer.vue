    ...
    <div v-if="order" class="fixed right-0 top-0 h-full w-full md:w-96 bg-white shadow-xl z-50">
      ...
      <div class="p-4 space-y-3 overflow-auto max-h-[75vh]">
        ...
        <div class="mt-4">
          ...
          <div>
            <BaseSelect v-model="localStatus" :options="[
        ...
        <div class="flex gap-2 mt-4">
          <button @click="applyStatus" class="px-3 py-2 bg-brand-500 text-white rounded-lg shadow">Durum Güncelle</button>
          <button @click="$emit('print', order)" class="px-3 py-2 border rounded-lg">Yazdır</button>
          <button v-if="order && order.table" @click="closeTableForOrder" class="px-3 py-2 bg-red-50 text-red-700 border border-red-200 rounded-lg">Masa Kapat</button>
        </div>
      </div>
    </div>
  </transition>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    async function closeTableForOrder() {
      ...
      try {
        await fetchJson('/api/orders/close-table/' + encodeURIComponent(props.order.table), { method: 'POST' })
