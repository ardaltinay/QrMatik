  ...
  <div>
    ...
    <div class="hidden md:block fixed right-6 bottom-6 w-80">
      <div class="bg-white border rounded-lg shadow-lg p-4 transform transition hover:shadow-2xl">
        ...
        <div class="mt-4">
          <button v-if="store.cart.length" @click="submitOrder" class="w-full px-4 py-2 bg-indigo-600 text-white rounded-md">Sipariş Ver</button>
    ...
    <div class="md:hidden fixed left-0 right-0 bottom-0 z-50">
      <transition name="slide-up" appear>
        <div v-if="mobileOpen" class="bg-white border-t rounded-t-xl shadow-xl p-4 motion-safe:animate-slideUp">
          ...
          <div class="mt-3">
            <button @click="submitOrder" class="w-full px-4 py-2 bg-indigo-600 text-white rounded-md">Sipariş Ver</button>
          </div>
        </div>
      </transition>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function submitOrder() {
      const tableCode = (() => { try { return localStorage.getItem('qm_table_code') } catch { return null } })()
      ...
      try { window.addEventListener('qm:orderSession', onCreated) } catch { /* ignore */ }
      // Fallback: if not received within 2s, go to session list
      setTimeout(() => {
        if (!navigated) {
          try { window.removeEventListener('qm:orderSession', onCreated) } catch { /* ignore */ }
          const sess = (() => { try { return localStorage.getItem('qm_order_session') } catch { return null } })()
  ...
  function clearCart() { store.clearCart() }
    ...
    function closeMobile() { emit('update:mobileOpen', false) }
...
.slide-up-enter-active { transition: transform 260ms cubic-bezier(.2,.8,.2,1), opacity 200ms }
.slide-up-enter-to { transform: translateY(0); opacity: 1 }
.slide-up-leave-from { transform: translateY(0); opacity: 1 }
.slide-up-leave-active { transition: transform 220ms cubic-bezier(.2,.8,.2,1), opacity 160ms }
