    ...
    <div v-if="order" class="space-y-4">
        ...
        <div class="mt-4">
          <ol class="grid grid-cols-2 md:flex md:items-center md:space-x-4 gap-3">
            <li v-for="(s, idx) in steps" :key="s.value" class="flex items-center gap-3">
              <div :class="[idx <= currentIdx ? 'bg-brand-500 text-white' : 'bg-gray-100 text-gray-600', 'flex items-center justify-center rounded-full']" :style="currentIdx >= idx ? mobileCircleFilledStyle : mobileCircleStyle">
                <span class="text-sm" v-if="idx < currentIdx || (currentIdx === steps.length - 1 && idx === currentIdx)">✓</span>
                <span class="text-sm" v-else>{{ idx + 1 }}</span>
              </div>
              <div class="text-sm" :class="idx <= currentIdx ? 'text-gray-800' : 'text-gray-500'">{{ s.label }}</div>
            </li>
          </ol>
        </div>
      </div>
      ...
      <div class="bg-white p-4 rounded-lg shadow">
        ...
        <div class="mt-4 flex items-center justify-between border-t pt-3">
          ...
          <div class="text-lg font-semibold">{{ formatMoney(order.total || sumItems(order.items)) }}</div>
        </div>
      </div>
      ...
      <div v-if="order && order.status === 'served' && !isExpired" class="mt-4 p-3 rounded border bg-blue-50 text-blue-900">
        <div class="mb-2">Siparişiniz servis edildi. İsterseniz ödemenizi burdan yapabilirsiniz.</div>
        <button @click="completePayment" class="px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Ödemeyi Yap</button>
      </div>
      <div v-else-if="order && order.status === 'payment_completed'" class="mt-4 p-3 rounded border bg-green-50 text-green-800">
        Ödeme tamamlandı. Afiyet olsun!
      </div>

      <div class="bg-white p-4 rounded-lg shadow flex gap-4 flex-col md:flex-row items-start md:items-center">
        <div class="md:w-48">
        ...
        <div class="flex-1">
          ...
          <div class="flex flex-col gap-2">
            <div class="flex gap-2">
              <button @click="copyLink" class="flex-1 px-3 py-2 bg-brand-500 text-white rounded">Linki Kopyala</button>
              <button @click="openQR" class="flex-1 px-3 py-2 border rounded">QR Aç</button>
            </div>
            <div class="flex flex-col md:flex-row md:gap-2">
              <button @click="sendEmail" class="flex-1 px-3 py-2 border rounded mb-2 md:mb-0">E-posta ile Gönder</button>
              <button @click="sendSMS" class="flex-1 px-3 py-2 border rounded">SMS ile Gönder</button>
            </div>
          </div>
          <div v-if="copied" class="text-sm text-green-600 mt-2">Link panoya kopyalandı.</div>
          <div class="text-sm text-gray-500 mt-2">Not: E-posta/SMS göndermek için gerçek sunucu entegrasyonu gerekir; butonlar varsayılan uygulamayı açar.</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  const order = computed(() => orderFromStore.value || loadedOrder.value)
    ...
    const etaText = computed(() => {
      ...
      if (status === 'preparing') return `${Math.max(1, baseMinutes - 2)} - ${baseMinutes + 2} dk içinde`
  ...
  const shareLink = window.location.origin + '/order/' + id
    ...
    function openQR() { window.open(qrUrl, '_blank') }
    ...
    function sumItems(items) {
      try { return (items || []).reduce((s, it) => s + (Number(it.price || 0) * (it.qty || 1)), 0) } catch { return 0 }
    }
  ...
  return { order, menuItemName, steps, currentIdx, etaText, showEta, isExpired, formatDate, orderCodeFromId, formatMoney, sumItems, shareLink, qrUrl, copyLink, openQR, sendEmail, sendSMS, copied, mobileCircleStyle, mobileCircleFilledStyle, completePayment }
  }
}
</script>