<template>
  <div class="max-w-3xl mx-auto p-4">
    ...
    <!-- Masa bilgisi -->
    <div v-if="orders.length && tableInfo" class="mb-4 p-3 rounded-lg bg-gray-50 border text-gray-700">
      <span class="font-medium">Masa:</span>
      <span>{{ tableInfo }}</span>
    </div>

    <div v-if="orders.length">
  <div v-for="o in orders" :key="o.id" class="bg-white p-4 rounded-lg shadow mb-4 cursor-pointer hover:shadow-md transition" @click="goDetail(o)">
...
const sessionId = ref(route.params.sessionId || (typeof localStorage !== 'undefined' ? localStorage.getItem('qm_order_session') : null))
const orders = ref([])
...
function menuItemName(id) { const i = store.menu.find(m => m.id === id); return i ? i.name : 'Unknown' }
function sumItems(items) { try { return (items || []).reduce((s, it) => s + (Number(it.price || 0) * (it.qty || 1)), 0) } catch { return 0 } }
function goDetail(o) {
  if (!o || !o.id) return
  try { store.ordersById[String(o.id)] = o } catch { /* ignore */ }
...
async function load() {
  if (!sessionId.value) { orders.value = []; return }
...
watch(() => route.params.sessionId, (v) => {
