<template>
  <div class="rounded bg-white p-4 shadow">
    ...
    <div v-if="!raw" class="text-sm text-gray-600">
      ...
      <div v-else>
      <div v-if="!opened" class="flex flex-col items-start gap-3 w-full">
        <p class="text-sm text-gray-600">Ödemeyi kapatırsanız plan yükseltme sayfasına geri dönebilirsiniz.</p>
        <p class="text-sm text-gray-600">Bu sayfada ya da yeni sekmede açabilirsiniz. Mobilde tarayıcılar yeni sekmeyi engelleyebilir.</p>
        <div class="flex flex-col gap-2 w-full sm:flex-row sm:gap-2 sm:w-auto">
          <button @click="openHere" class="btn btn-primary w-full sm:w-auto">Bu Pencerede Aç</button>
          <button v-if="!isMobile" @click="openWindow" class="btn btn-primary" style="background-image:none;background-color:#f59e0b">Yeni Sekmede Aç</button>
          <button v-if="!isMobile && canBlobFallback" @click="openWithBlob" class="btn btn-secondary">Alternatif Aç (Blob)</button>
          <button @click="goBackToUpgrade" class="btn btn-secondary w-full sm:w-auto">Vazgeç ve Geri Dön</button>
        </div>
      </div>
      <div v-else class="flex items-center justify-between gap-3 text-sm text-gray-600">
        <span>Ödeme penceresi açıldı. Görmüyorsanız tarayıcınızın pop-up engelleyicisini kontrol edin.</span>
  ...
  // Not auto-opening anymore; flag removed
    ...
    function buildHtml(content) {
      ...
      const looksLikeFullDoc = /<html[\s>]/i.test(trimmed) || /<!doctype/i.test(trimmed);
        const bar = "<div id=\"qm-topbar\" style=\"position:sticky;top:0;left:0;right:0;background:#f8fafc;border-bottom:1px solid #e5e7eb;padding:8px 12px;display:flex;justify-content:space-between;align-items:center;z-index:9999;font-family:ui-sans-serif,system-ui,-apple-system;\">" +
                    "<strong>Ödeme</strong>" +
        ...
        if (looksLikeFullDoc) {
          ...
          try {
            return trimmed.replace(/<body[^>]*>/i, function(m){ return m + bar + guardScript; });
          } catch (e) {
        ...
        return (
          ...
          "</head><body>" +
          bar + guardScript +
          "<div style=\"padding:12px\">" + trimmed + "</div>" +
          "</body></html>"
        );
    }
    ...
    function openWindow() {
      ...
      try {
        ...
        setTimeout(() => {
          try {
            const bodyLen = (w.document && w.document.body && w.document.body.innerHTML || '').trim().length;
    ...
    function openHere() {
      ...
      try {
        ...
        try { sessionStorage.setItem('qm_checkout_auto_opened', '1'); } catch (e) { /* noop */ }
        document.open();
        document.write(html);
        document.close();
      } catch {
    ...
    function goBackToUpgrade() {
      try { sessionStorage.removeItem('qm_checkout_content'); } catch (e) { /* noop */ }
      router.replace('/admin/upgrade');
    }

    function goBackAfterOpened() {
      try { if (popupRef.value && !popupRef.value.closed) popupRef.value.close(); } catch (e) { /* noop */ }
    ...
    onMounted(() => {
      ...
      try {
        ...
        isMobile.value = /iPhone|Android|Mobile/i.test(navigator.userAgent || '') || (typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(max-width: 640px)').matches);
      } catch (e) { /* noop */ }
      try { sessionStorage.removeItem('qm_checkout_auto_opened'); } catch (e) { /* noop */ }
      ...
      pollTimer = window.setInterval(() => {
        try {
          ...
          if (w && w.closed) {
            let res = null;
            try { res = localStorage.getItem('qm_upgrade_result'); localStorage.removeItem('qm_upgrade_result'); } catch (e) { /* noop */ }
    ...
    // Cleanup
    onBeforeUnmount(() => {
      try { window.removeEventListener('message', onMsg); } catch (e) { /* noop */ }
      try { if (pollTimer) window.clearInterval(pollTimer); } catch (e) { /* noop */ }
    });

    return { raw, container, opened, openWindow, openWithBlob, canBlobFallback, openHere, goBackToUpgrade, goBackAfterOpened };
  },
};
</script>
