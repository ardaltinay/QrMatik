      ...
      >
        <div class="flex items-start justify-between gap-3">
          <div>
            ...
            <div class="text-sm text-gray-500" v-if="!editing[it.id]">
        ...
        <div class="flex items-center gap-4 flex-wrap">
          ...
          <div class="text-xs text-amber-600" v-else-if="!isUuid(it.id)">
            Önce ürünü sunucuda oluşturup kaydedin (geçersiz ID).
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      const cfg = (() => {
        try {
          return tenantCfg && tenantCfg.config ? JSON.parse(tenantCfg.config) : null;
        } catch {
          return null;
        }
      })();
      const categoryOptions = computed(() =>
        cfg && Array.isArray(cfg.categories) && cfg.categories.length
          ? cfg.categories
      ...
      function addItem() {
        ...
        if (price.value == null || Number(price.value) < 0) {
          statuses["_new"] = "Fiyat 0 veya üzeri olmalı";
          setTimeout(() => {
            statuses["_new"] = "";
          }, 2000);
          return;
      ...
      async function upload(id) {
        ...
        try {
          ...
          ).apiFetch(`/api/menu/${id}/image`, { method: "POST", body: form });
          if (!res.ok) {
      ...
      function startEdit(it) {
        ...
        drafts[it.id] = {
          name: it.name,
          price: it.price,
          category: it.category || it.primary,
          subcategory: it.sub || it.subcategory,
        };
      }
      ...
      async function saveEdit(id) {
        ...
        try {
          ...
          try {
            const { useUiStore } = await import("@/stores/uiStore");
            useUiStore().toastError(statuses[id]);
          } catch (e2) {
            /* ignore */
          }
        } finally {
