...
async function saveStock(item: any) {
  ...
  try {
    ...
    // But we only want to update the stock fields so we just send the whole item again,
    // or just assume the backend merges it. The standard is to send the full item.
    const payload = {
      ...item,
    ...
    // Use toast if available, or just silent success
