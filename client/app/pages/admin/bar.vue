  ...
  <div class="p-4 md:p-8 max-w-[1800px] mx-auto h-[calc(100vh-64px)] sm:h-screen overflow-hidden flex flex-col space-y-6">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 shrink-0 bg-white/50 backdrop-blur-xl p-6 rounded-[2.5rem] border border-white shadow-xl shadow-slate-200/40">
      <div class="flex items-center gap-5">
        ...
        <div>
          <h1 class="text-3xl font-black text-slate-900 tracking-tight leading-none mb-2">{{ $t('admin.bar.title') }}</h1>
          <p class="text-slate-500 font-medium text-sm">{{ $t('admin.bar.subtitle') }}</p>
        </div>
      </div>

      <div class="flex items-center gap-3 w-full lg:w-auto">
        <div class="relative flex-1 lg:min-w-[400px] group">
          <svg class="w-5 h-5 absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
          ...
          <input 
            ...
            class="w-full pl-14 pr-6 py-4 rounded-2xl bg-white border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all outline-none shadow-sm font-bold text-slate-700"
          />
        </div>
    ...
    <div class="flex-grow overflow-x-auto pb-6 flex gap-8 items-start scrollbar-hide">
      ...
      <div class="w-full min-w-[320px] max-w-[450px] shrink-0 flex flex-col h-full max-h-full">
        <div class="flex items-center justify-between mb-5 px-4 bg-white/70 backdrop-blur-md py-4 rounded-2xl border border-white shadow-sm">
          ...
          <span class="bg-amber-500 text-white text-[10px] font-black px-3 py-1.5 rounded-xl shadow-lg shadow-amber-500/20">{{ newOrders.length }}</span>
        </div>
        ...
        <div class="flex-grow overflow-y-auto space-y-4 pr-2 scrollbar-thin">
          <TransitionGroup name="list">
            <div 
              ...
              :key="order.id"
              class="bg-white p-6 rounded-[2rem] shadow-xl shadow-slate-200/40 border border-slate-100 hover:border-amber-300 transition-all relative overflow-hidden group/card animate-pulse-subtle"
            >
              <div class="absolute top-0 left-0 w-2 h-full bg-amber-500"></div>
              
              <div class="flex justify-between items-start mb-4">
                <div>
                   <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">BAR SİPARİŞİ</span>
              ...
              <div class="space-y-3 mb-6 bg-slate-50/50 p-4 rounded-2xl border border-slate-100 shadow-inner">
                <div v-for="(it, i) in filterItems(order.items || order.lines)" :key="i" class="flex items-start gap-3">
                  <div class="bg-indigo-600 text-white w-7 h-7 rounded-lg flex items-center justify-center font-black text-sm shrink-0 shadow-md shadow-indigo-500/20">
                  ...
                  <div class="flex-1">
                    ...
                    <p v-if="it.note" class="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded-md mt-1.5 inline-block border border-amber-100">
                      ⚠️ {{ it.note }}
                    </p>
                  </div>
                </div>
              ...
              <button @click="updateStatus(order.id, 'preparing')" class="w-full py-4.5 bg-indigo-600 text-white font-black text-sm uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-indigo-500/30 hover:bg-indigo-700 active:scale-[0.98] transition-all">
        ...
        <div class="flex-grow overflow-y-auto space-y-4 pr-2 scrollbar-thin">
          <TransitionGroup name="list">
            ...
            >
              ...
              <div class="flex justify-between items-start mb-4">
                <div>
                   <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">HAZIRLANIYOR</span>
                   <span class="text-2xl font-black text-slate-900 tracking-tighter">#{{ orderCodeFromId(order.id) }}</span>
                </div>
                <div class="text-right">
              ...
              <button @click="updateStatus(order.id, 'ready')" class="w-full py-4.5 bg-emerald-600 text-white font-black text-sm uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-emerald-500/30 hover:bg-emerald-700 active:scale-[0.98] transition-all">
          ...
          <TransitionGroup name="list">
            ...
            >
              ...
              <div class="flex justify-between items-center">
                 <div>
                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">HAZIR</span>
...
async function updateStatus(id: string | number, status: string) {
  try { await orderStore.updateOrderStatus(id, status, 'BAR') }
  catch (e: any) { uiStore.error(e?.message || 'Hata oluştu') }
}

useHead({ title: () => `${t('admin.bar.title')} | Admin` })
</script>
...
@keyframes pulse-subtle {
  ...
  50% { transform: scale(1.01); box-shadow: 0 20px 40px -10px rgba(245, 158, 11, 0.2); border-color: rgba(245, 158, 11, 0.3); }
}
